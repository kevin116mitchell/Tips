...
stopwords = ["a", "about", "above", "above", "across", "after", "afterwards", "again", 
             ...
             "yourself", "yourselves", "the", "i", "r","dr", "s","im", "k","ill","ons","ls","nd","th","app","hw","sa",
             "hr", "ate", "ran","ace","war","stop", "oh"]
...
cells_PD = []
cells_LS = []
labels_strings = ["Self Improvement        :", "Social advice           :", "Professional Development:", 
        "Learning Strategies     :"]
# =============================================================================


#A function to make printing the data easier
def stringMaker(label_list):
    temp_string = "\t"
    for i in range(6):
        temp_string += str(len(label_list[i])) + "\t\t"
    return temp_string

#A function to make printing the data easier
def stringMaker_percentage(percentages):
    temp_string = "\t"
    for i in range(6):
        temp_string += str(percentages[i]) + "\t\t"
    return temp_string
    ...
    for cell in all_cells:
        ...
        for word in word_list:
            ...
            for key in faculty_dict.keys():
                if word in str(key).lower() and word not in stopwords:
                    print("WARNINGGGGGGGG" +word)
                    faculty_dict[key] += 1
...
labels = [cells_SI,cells_SA,cells_PD,cells_LS]
y = 0
...
for label in labels:
    ...
    for year in label:
        total_tips+= len(year)
        temp_array.append(len(year))
        
    i = 0
    for year in label:
        percentages.append(round(temp_array[i]/total_tips,2))
        i+=1
        
    print(str(labels_strings[y]) + stringMaker_percentage(percentages))
    y+=1
